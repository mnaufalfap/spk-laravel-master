...
<div class="card-body">
    ...
    <hr><br>
    <form class="forms-sample" action="{{ route('admin.bobot.store') }}" method="POST">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="number" id="put{{ $kriteria->id }}" style="width:3rem;" value="10" readonly>
            <script>
                ...
                @foreach ($bobot as $bobot)
                    ...
                    <div class="modal fade" id="editbobot-{{ $bobot->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <form method="POST" action="{{ route('admin.bobot.update', $bobot->id) }}">
                                @csrf
                                <input type="hidden" name="_method" value="PUT">
                                <div class="form-group">
                                    <label for="{{ $bobot->Kriteria->id }}"><h4>{{ $bobot->Kriteria->nama_kriteria }}</h4></label>
                                    <p class="card-secription">Seberapa penting {{ $bobot->Kriteria->nama_kriteria }} terhadap lansia ?</p>
                                    <input class="progress" type="range" name="point" id="get{{ $bobot->Kriteria->id }}" min="10" max="100" value="{{ $bobot->point }}" onchange="bobot{{ $bobot->Kriteria->id }}()" step="10" required>
